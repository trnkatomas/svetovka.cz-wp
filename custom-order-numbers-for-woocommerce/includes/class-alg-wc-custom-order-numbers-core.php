...
class Alg_WC_Custom_Order_Numbers_Core {
    ...
    /**
	* Custom function parsing and showing date prefix
	*/

	public function replace_date_placeholders( $ord_number ){
		$re = '/(%[YyMmndj])/m'; 
		$output = preg_split($re, $ord_number, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
		
		$out_string = "";

		foreach ( $output as $key => $value ){
    		if (strlen($value) === 2 && substr($value,0,1) === "%"){
        		$out_string .= date(substr($value,1,1));
    		}else{
	        	$out_string .= $value;
    		}
		}
	...
	function display_order_number( $order_number, $order ) {
		...
		$order_number = apply_filters( 'alg_wc_custom_order_numbers',
      									sprintf( '%s',
      											  // comented out because of the date I'm storing it directly into database	
                                                  //do_shortcode( get_option( 'alg_wc_custom_order_numbers_prefix', '' ) ),
      											  $order_number_meta ),
      									'value',
      									array( 'order_timestamp' => $order_timestamp,
	...
	function add_order_number_meta( $order_id, $do_overwrite ) {
		...
		if ( true === $do_overwrite || 0 == get_post_meta( $order_id, '_alg_wc_custom_order_number', true ) ) {
			...
			$output = preg_split($re, $prefix, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
			...
			if ( 'sequential' === $counter_type ) {
				...
				if ( NULL != $result_select ) {
					...
					if ( NULL != $result_update || $result_select->option_value == ( $current_order_number + 1 ) ) {
						...
						update_post_meta( $order_id, '_alg_wc_custom_order_number', $out_string . $current_order_number );
					} else {
